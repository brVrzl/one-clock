#!/usr/bin/env python3
"""Run the preregistered, resumable Gate-3B LIBERO composition rollouts."""
...
from gate3b_composition import (  # noqa: E402
    ...
    FixedAgeComposer,
    control_semantic_distance,
)
from gate3b_schedule import pending_runs  # noqa: E402
...
OUTPUT_ROOT = ROOT / "experiments/gate3b_cross_generation_composition"
...
def provenance(checkpoint: Path, config_path: Path, schedule_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "composition_temporal_ensemble_active": TEMPORAL_ENSEMBLE_ACTIVE,
...
def make_env_and_policy(
    ...
    if policy.config.temporal_ensemble_coeff is not None:
        raise RuntimeError("Gate-3B requires policy-internal temporal ensembling to be disabled")
...
def mean_record_value(records: list[dict[str, Any]], key: str) -> float:
...
def summarize_action_sequence(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    intervention = [record for record in records if record["intervention_active"]]
    summary: dict[str, Any] = {
        "intervention_active_steps": len(intervention),
        "mixed_source_steps": int(sum(bool(record["cross_generation_composition"]) for record in records)),
        "mean_arm_source_age_ticks": mean_record_value(records, "arm_source_age_ticks"),
        "mean_gripper_source_age_ticks": mean_record_value(records, "gripper_source_age_ticks"),
        "mean_fresh_old_arm_l2": mean_record_value(intervention, "fresh_old_arm_l2"),
        "mean_fresh_old_translation_l2": mean_record_value(intervention, "fresh_old_translation_l2"),
        "mean_fresh_old_rotation_radians": mean_record_value(
            intervention, "fresh_old_rotation_radians"
        ),
        "mean_fresh_old_gripper_sign_disagreement": mean_record_value(
            intervention, "fresh_old_gripper_sign_disagreement"
        ),
        "mean_distance_to_fresh_joint_source_action": mean_record_value(
            intervention, "distance_to_fresh_joint_source_action"
        ),
        "mean_distance_to_old_joint_source_action": mean_record_value(
            intervention, "distance_to_old_joint_source_action"
        ),
        "mean_distance_to_nearest_jointly_predicted_source_action": mean_record_value(
            intervention, "distance_to_nearest_jointly_predicted_source_action"
        ),
...
def run_episode(
    *,
    ...
    for step in range(MAX_STEPS):
        ...
        composed = composer.update(step, chunk)
        fresh_distance = control_semantic_distance(composed.action, composed.fresh_action)
        if composed.old_action is None:
            old_distance = None
            fresh_old_distance = None
            nearest_distance = fresh_distance["dimension_weighted_semantic_distance"]
        else:
            old_distance = control_semantic_distance(composed.action, composed.old_action)
            fresh_old_distance = control_semantic_distance(composed.fresh_action, composed.old_action)
            nearest_distance = min(
                fresh_distance["dimension_weighted_semantic_distance"],
                old_distance["dimension_weighted_semantic_distance"],
            )
        observation, reward, terminated, truncated, info = env.step(composed.action)
        records.append(
            {
                "step": step,
                "method": method,
                "action": composed.action.astype(float).tolist(),
                "fresh_action": composed.fresh_action.astype(float).tolist(),
                "old_action": None
                if composed.old_action is None
                else composed.old_action.astype(float).tolist(),
                "intervention_active": composed.intervention_active,
                "cross_generation_composition": bool(
                    composed.intervention_active and method in ("FO", "OF")
                ),
                "fresh_source_step": composed.fresh_source_step,
                "old_source_step": composed.old_source_step,
                "fresh_chunk_offset": composed.fresh_chunk_offset,
                "old_chunk_offset": composed.old_chunk_offset,
                "arm_source_step": composed.arm_source_step,
                "gripper_source_step": composed.gripper_source_step,
                "arm_source_age_ticks": step - composed.arm_source_step,
                "gripper_source_age_ticks": step - composed.gripper_source_step,
                "fresh_old_arm_l2": 0.0
                if fresh_old_distance is None
                else fresh_old_distance["raw_arm_l2"],
                "fresh_old_translation_l2": 0.0
                if fresh_old_distance is None
                else fresh_old_distance["translation_l2_action_units"],
                "fresh_old_rotation_radians": 0.0
                if fresh_old_distance is None
                else fresh_old_distance["rotation_geodesic_radians"],
                "fresh_old_gripper_sign_disagreement": 0.0
                if fresh_old_distance is None
                else fresh_old_distance["gripper_sign_disagreement"],
                "fresh_old_control_semantic_distance": None
                if fresh_old_distance is None
                else fresh_old_distance["dimension_weighted_semantic_distance"],
                "distance_to_fresh_joint_source_action": fresh_distance[
                    "dimension_weighted_semantic_distance"
                ],
                "distance_to_old_joint_source_action": None
                if old_distance is None
                else old_distance["dimension_weighted_semantic_distance"],
                "distance_to_nearest_jointly_predicted_source_action": nearest_distance,
                "query_seconds": query_elapsed,
                "reward": float(reward),
    ...
    if policy_queries != steps:
        raise RuntimeError("Gate-3B requires policy_queries == environment_steps")
    summary = {
...
def manifest_entry(path: Path, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    entry = {
        **run,
        ...
        "intervention_active_steps": int(summary["intervention_active_steps"]),
        "mixed_source_steps": int(summary["mixed_source_steps"]),
    ...
    entry.update({field: float(summary[field]) for field in fields})
    return entry
...
def write_manifest(
    ...
    *,
    ...
    atomic_json(
        ...
        {
            "schema_version": 1,
            "scope": "Gate-3B preregistered cross-generation composition experiment",
...
def main() -> None:
    ...
    if int(schedule.get("source_age_ticks", -1)) != SOURCE_AGE_TICKS:
        raise RuntimeError("schedule source age differs from the fixed d=20 contract")
    ...
    write_manifest(args.manifest, schedule=schedule, provenance_data=provenance_data, entries=entries)
