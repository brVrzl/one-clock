#!/usr/bin/env python3
"""Run the preregistered, resumable Gate-3C LIBERO rollouts."""
...
from gate3c_schedule import pending_runs  # noqa: E402
...
SCHEDULE = ROOT / "research/audit_outputs/gate3c_run_schedule.json"
OUTPUT_ROOT = ROOT / "experiments/gate3c_asymmetric_temporal_reuse"
...
def provenance(checkpoint: Path, config_path: Path, schedule_path: Path) -> dict[str, Any]:
    observed_model_hash = sha256(checkpoint / "model.safetensors")
    observed_config_hash = sha256(checkpoint / "config.json")
...
def make_env_and_policy(config: dict[str, Any], checkpoint: Path, task_id: int) -> tuple[Any, ...]:
    ...
    if policy.config.temporal_ensemble_coeff is not None:
        raise RuntimeError("Gate-3C requires policy-internal temporal ensembling to be disabled")
    if int(policy.config.chunk_size) != CHUNK_LENGTH or int(policy.config.action_feature.shape[0]) != ACTION_DIM:
        raise RuntimeError("checkpoint chunk/action shape differs from preregistration")
    env = LiberoEnv(
        task_suite=suite,
        task_id=task_id,
    ...
    if env.control_freq != CONTROL_HZ or int(env._max_episode_steps) != MAX_STEPS:
        raise RuntimeError("runtime LIBERO time contract differs from preregistration")
...
def summarize(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    result = {
        ...
        "mean_rotation_action_delta_radians": float(np.mean([
        ...
        ) if len(actions) > 2 else 0.0,
        "mean_raw_action_jerk_l2": float(
            np.linalg.norm(np.diff(actions, n=3, axis=0), axis=1).mean()
        ) if len(actions) > 3 else 0.0,
    }
    return result


def run_episode(
    *, env: Any, policy: Any, policy_preprocessor: Any, policy_postprocessor: Any,
    env_preprocessor: Any, env_postprocessor: Any, run: dict[str, Any]
) -> dict[str, Any]:
    import torch
        ...
        records.append(
            {
                ...
                "old_action": None if executed.old_action is None else executed.old_action.astype(float).tolist(),
                ...
                "scalar_weights": None if np.isnan(executed.weights).any() else executed.weights.tolist(),
                "fresh_old_gripper_sign_disagreement": 0.0 if disagreement is None else disagreement["gripper_sign_disagreement"],
                "fresh_old_translation_l2": 0.0 if disagreement is None else disagreement["translation_l2_action_units"],
                "fresh_old_rotation_radians": 0.0 if disagreement is None else disagreement["rotation_geodesic_radians"],
                "query_seconds": query_elapsed,
                "reward": float(reward),
...
def episode_path(output_root: Path, run: dict[str, Any]) -> Path:
    return output_root / "episodes" / f"task_{int(run['task_id']):02d}" / f"state_{int(run['state_id']):02d}" / f"{run['method']}.json.gz"
...
def read_valid_episode(path: Path, run: dict[str, Any], provenance_data: dict[str, Any]) -> dict[str, Any]:
    ...
    if int(payload["summary"]["policy_queries"]) != int(payload["summary"]["steps"]):
        raise RuntimeError(f"existing episode query cadence mismatch: {path}")
    return payload
...
def write_manifest(path: Path, schedule: dict[str, Any], provenance_data: dict[str, Any], entries: dict[int, dict[str, Any]]) -> None:
    ordered = [entries[index] for index in sorted(entries)]
    atomic_json(path, {
        "schema_version": 1,
        "scope": "Gate-3C preregistered asymmetric temporal reuse confirmation",
...
def main() -> None:
    ...
    if schedule.get("planned_episodes") != 700 or tuple(schedule.get("methods", [])) != METHODS:
        raise RuntimeError("schedule differs from frozen 700-episode method set")
    ...
    if args.validate_runtime:
        ...
        print("validated frozen Gate-3C runtime without generating an outcome")
    if args.verify_only:
    ...
    completed_new = 0
    try:
        for run in pending_runs(schedule, set(entries), args.max_new_runs):
            ...
            payload = run_episode(
                env=env, policy=policy, policy_preprocessor=policy_pre,
                policy_postprocessor=policy_post, env_preprocessor=env_pre,
                env_postprocessor=env_post, run=run,
            )
            path = episode_path(args.output_root, run)
