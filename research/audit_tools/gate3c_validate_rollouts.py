#!/usr/bin/env python3
"""Post-result integrity validation for Gate-3C rollouts."""
...
OUTPUT = ROOT / "research/audit_outputs/gate3c_rollout_validation.json"
ROLLOUT_ROOT = ROOT / "experiments/gate3c_asymmetric_temporal_reuse"
...
def main() -> None:
    ...
    if not manifest["complete"] or manifest["completed_episodes"] != 700:
    ...
    if len(expected_runs) != 700:
        raise RuntimeError("frozen schedule does not contain 700 unique cells")
    ...
    for episode in manifest["episodes"]:
        ...
        for step, record in enumerate(records):
            ...
            if method in METHODS[:3]:
                old = None if record["old_action"] is None else np.asarray(record["old_action"], dtype=np.float64)
    ...
    if max_fixed_formula_error != 0.0 or max_fixed_prefix_difference != 0.0:
        raise RuntimeError("fixed formula or first-20 prefix validation failed")
    observed_cells = {
    ...
    output: dict[str, Any] = {
        ...
        "complete_unique_task_state_method_cells": 700,
        "task_state_blocks": len(prefixes),
        "total_step_records": total_steps,
        "total_policy_queries": manifest["valid_policy_queries"],
        "all_queries_equal_environment_steps": total_steps == manifest["valid_policy_queries"] == manifest["valid_environment_steps"],
