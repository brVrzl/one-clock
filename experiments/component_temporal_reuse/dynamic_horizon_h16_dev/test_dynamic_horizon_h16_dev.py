...
import run_dynamic_horizon_h16_dev as runner  # noqa: E402
...
def test_adaptive_h16_age_trigger_is_exactly_sixteen() -> None:
    chunk = np.zeros((32, 7), dtype=float)
    records = [runner.QueryRecord(0, chunk)]
    query, reasons, _ = runner.adaptive_query_decision(records, 16)
    assert query and "query_age_ge_16" in reasons
    query, reasons, _ = runner.adaptive_query_decision(records, 15)
    assert query and "query_age_ge_16" not in reasons


def test_fixed_h16_queries_only_on_period_boundary() -> None:
    records = []
