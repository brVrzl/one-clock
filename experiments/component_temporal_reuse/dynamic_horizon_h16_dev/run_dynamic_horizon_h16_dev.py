#!/usr/bin/env python3
"""Run the second frozen ACT sparse-query dynamic-horizon development panel."""
...
METHODS = ("fixed_h16", "adaptive_h16_cos09")
MAX_AGE = 16
...
def adaptive_query_decision(records: list[QueryRecord], target_step: int) -> tuple[bool, list[str], dict]:
    ...
    if candidates and candidates[-1][0] >= MAX_AGE:
        reasons.append("query_age_ge_16")
    if len(candidates) >= 2:
...
def scheduler_step(method: str, target_step: int, records: list[QueryRecord], query_fn):
    ...
    if method == "fixed_h16":
        should_query, reasons = target_step % 16 == 0, (["fixed_period"] if target_step % 16 == 0 else [])
...
def semantic_smoke() -> None:
    ...
    chunks = [np.arange(40 * 7, dtype=np.float64).reshape(40, 7) + 100.0 * step for step in (0, 16, 32)]
    ...
    action, queried, reasons, _, age, source = scheduler_step("fixed_h16", 33, records, lambda: chunks[2])
    assert not queried and age == 1 and source == 32
    ...
    print(json.dumps({"status": "dynamic_horizon_h16_cpu_semantic_smoke_pass", "methods": list(METHODS)}))
...
def rollout_episode(*, env, policy, infer_chunk, processors, torch, method, seed, state_id, policy_rng_seed, max_steps):
    ...
    trigger_counts = {"fixed_period": 0, "insufficient_candidates": 0, "query_age_ge_16": 0, "gripper_sign_disagreement": 0, "arm_cosine_lt_0.90": 0}
    success = False
