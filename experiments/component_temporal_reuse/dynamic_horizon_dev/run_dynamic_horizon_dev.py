#!/usr/bin/env python3
"""Run the frozen ACT sparse-query dynamic-horizon development panel."""
...
METHODS = ("fixed_h4", "fixed_h8", "adaptive_h8_cos09")
...
def _arm_cosine(first: np.ndarray, second: np.ndarray) -> float:
    first = np.asarray(first, dtype=np.float64)
    second = np.asarray(second, dtype=np.float64)
    denominator = np.linalg.norm(first) * np.linalg.norm(second) + 1e-7
    return float(first @ second / denominator)


def adaptive_query_decision(records: list[QueryRecord], target_step: int) -> tuple[bool, list[str], dict]:
    """Make the frozen pre-query decision from sparse records only."""

    candidates = _candidate_records(records, target_step)
    reasons: list[str] = []
    diagnostics = {"candidate_count": len(candidates), "nearest_query_age": None, "arm_cosine": None, "gripper_sign_disagreement": False}
    if candidates:
        diagnostics["nearest_query_age"] = int(candidates[-1][0])
    if len(candidates) < 2:
        reasons.append("insufficient_candidates")
    if candidates and candidates[-1][0] >= 8:
        reasons.append("query_age_ge_8")
    if len(candidates) >= 2:
        previous = candidates[-2][1]
        newest = candidates[-1][1]
        sign_disagreement = bool(np.sign(previous[6]) != np.sign(newest[6]))
        arm_cosine = _arm_cosine(previous[:6], newest[:6])
        diagnostics["gripper_sign_disagreement"] = sign_disagreement
        diagnostics["arm_cosine"] = arm_cosine
        if sign_disagreement:
            reasons.append("gripper_sign_disagreement")
        if arm_cosine < 0.90:
            reasons.append("arm_cosine_lt_0.90")
...
def scheduler_step(method: str, target_step: int, records: list[QueryRecord], query_fn):
    ...
    if method == "fixed_h4":
        should_query, reasons = target_step % 4 == 0, (["fixed_period"] if target_step % 4 == 0 else [])
        diagnostics = {"candidate_count": len(_candidate_records(records, target_step))}
    elif method == "fixed_h8":
        should_query, reasons = target_step % 8 == 0, (["fixed_period"] if target_step % 8 == 0 else [])
        diagnostics = {"candidate_count": len(_candidate_records(records, target_step))}
    else:
        should_query, reasons, diagnostics = adaptive_query_decision(records, target_step)
    if should_query:
...
def semantic_smoke() -> None:
    ...
    chunks = [np.arange(20 * 7, dtype=np.float64).reshape(20, 7) + 100.0 * step for step in (0, 4, 8)]
    records = [QueryRecord(0, chunks[0]), QueryRecord(4, chunks[1]), QueryRecord(8, chunks[2])]
    candidates = _candidate_records(records, 8)
    assert [row[0] for row in candidates] == [8, 4, 0]
    assert [row[2] for row in candidates] == [0, 4, 8]
    action, queried, reasons, _, age, source = scheduler_step("fixed_h8", 9, records, lambda: chunks[2])
    assert not queried and age == 1 and source == 8
    np.testing.assert_array_equal(action, chunks[2][1])

    # Adaptive: first step must query; then disagreement/cosine triggers are
    # independently visible before any new record is appended.
    adaptive_records = [QueryRecord(0, np.tile(np.asarray([1, 0, 0, 0, 0, 0, 1.0]), (20, 1)))]
    query, reasons, diagnostics = adaptive_query_decision(adaptive_records, 1)
    assert query and "insufficient_candidates" in reasons
    adaptive_records.append(QueryRecord(1, np.tile(np.asarray([-1, 0, 0, 0, 0, 0, -1.0]), (20, 1))))
    query, reasons, diagnostics = adaptive_query_decision(adaptive_records, 2)
    assert query and "gripper_sign_disagreement" in reasons and "arm_cosine_lt_0.90" in reasons
    assert diagnostics["candidate_count"] == 2
    print(json.dumps({"status": "dynamic_horizon_cpu_semantic_smoke_pass", "methods": list(METHODS)}))
...
def rollout_episode(*, env, policy, infer_chunk, processors, torch, method, seed, state_id, policy_rng_seed, max_steps):
    ...
    trigger_counts = {"fixed_period": 0, "insufficient_candidates": 0, "query_age_ge_8": 0, "gripper_sign_disagreement": 0, "arm_cosine_lt_0.90": 0}
    success = False
...
def main() -> None:
    ...
    parser.add_argument("--live-smoke", action="store_true", help="run one state per method instead of full 30-episode task shard")
