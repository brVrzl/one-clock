...
def main() -> None:
    ...
    for policy in payloads:
        ...
        for offset in range(33):
            ...
            for dim in range(7):
                ...
                tidy.append({"policy": policy, "offset": offset, "quantity": "normalized_rmse", "metric": f"dim_{dim}", **record})
            for group, dims in GROUPS.items():
                ...
                tidy.append({"policy": policy, "offset": offset, "quantity": "normalized_rmse", "metric": group, **record})
            ...
            tidy.append({"policy": policy, "offset": offset, "quantity": "gripper_sign_disagreement", "metric": "gripper", "probability": grip_center, "episode_cluster_bootstrap_ci": ci(grip_boot)})
    ...
    output = {
        ...
        "offsets": b3["offsets"], "bootstrap": {"unit": b3["bootstrap_unit"], "draws": draws, "ACT_seed": b3["ACT_seed"], "SmolVLA_seed": b3["SmolVLA_seed"]},
