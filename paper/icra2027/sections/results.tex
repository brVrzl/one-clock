...
\subsection{Q1: Same-Target Arm--Gripper Asymmetry}
...
\subsection{Q2: Temporal Scale and Component Identity}
...
cohort (Fig.~\ref{fig:development}, left; Table~\ref{tab:r1a-full}). Observed separations ranged from
...
arm/fresh gripper; FA/SG denotes fresh arm/stale gripper. Separations are
FA/SG minus SA/FG in percentage points.}
\label{tab:r1a-full}
\scriptsize
\setlength{\tabcolsep}{2pt}
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}rrccc}
\toprule
$d$ & Time & SA/FG & FA/SG & Separation \\
\midrule
...
\subsection{Q4: Executable Consequences and Query-Matched Controls}
...
Interaction estimates remain supporting evidence and are not pooled. The
original 140-block post-study interaction was $+15.71$ points, with paired-block
interval $[+6.43,+25.00]$, task-$t$ interval $[-2.76,+34.18]$, and exact task
sign-flip $p=0.101562$. The R1C interaction was fixed before its outcomes and
was $+9.29$ points, but no canonical interaction-specific interval, $p$-value,
or leave-one-out estimate exists. Spatial has no named canonical interaction
estimate. These point estimates do not establish replicated interaction. The
R1C estimate used matched policy-query schedules, so the observed conditional
pattern is not simply created by one condition querying more often.
