...
LeRobot implementation~\cite{cadene2024lerobot}, with chunk size $100$,
...
\textsc{hard-h16}: the policy is queried once every $16$ steps and both groups
read the current committed chunk at offset $t-q$. Its source ages span
0--15 steps, with an observed mean of approximately $7.4$ steps in our
rollouts; \textsc{hard-h16} is thus not a fully reactive baseline.

Two executor variants isolate which group carries the commitment.
\textsc{C2} issues a dense whole-policy query at every control step and takes
only its offset-$0$ gripper component, while the arm remains sourced from the
current scheduled h16 chunk. \textsc{C1} gives the arm the same schedule while
the gripper reads the previous committed chunk; for the initial $16$-step
prefix, where no previous chunk exists, \textsc{C1} falls back to the current
chunk, after which its gripper source age lies in $16$--$31$ steps.
...
LIBERO-10 tasks $0,2,4,6,7$ over initial states $0$--$13$, giving $140$
